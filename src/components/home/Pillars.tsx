...
export function Pillars() {
    ...
    () => {
      ...
      mm.add(
        ...
        () => {
          ...
          // Snap points = the left edge of each panel, as scroll progress.
          const panels = gsap.utils.toArray<HTMLElement>(".pillar-panel");
          const computePoints = () => {
            const len = getScrollLen() || 1;
            return panels.map((p) =>
              gsap.utils.clamp(0, 1, p.offsetLeft / len),
            );
          };
  ...
  return (
    <section
      ref={root}
      className="relative overflow-hidden border-t border-white/[0.06] bg-void"
    >
      <div
        ref={track}
      ...
      >
        {/* Intro panel */}
        <div className="pillar-panel flex shrink-0 flex-col justify-center px-(--spacing-gutter) py-20 md:h-full md:w-[70vw] md:py-0 lg:w-[46vw]">
